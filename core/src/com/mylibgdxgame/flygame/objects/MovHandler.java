...
public class MovHandler {
    ...
    public boolean collides(Tartaglia tartaglia) {
        if (!web1.isScored()
                    && web1.getX() + (web1.getWidth() / 2) < tartaglia.getX()
                    + tartaglia.getWidth()) {
                addScore(1);
                web1.setScored(true);
                ResourseLoader.coin.play();
            } else if (!web2.isScored()
                    && web2.getX() + (web2.getWidth() / 2) < tartaglia.getX()
                    + tartaglia.getWidth()) {
                addScore(1);
                web2.setScored(true);
                ResourseLoader.coin.play();
                    ...
                    && web3.getX() + (web3.getWidth() / 2) < tartaglia.getX()
                    + tartaglia.getWidth()) {
                addScore(1);
                web3.setScored(true);
                ResourseLoader.coin.play();
       ...
       return (web1.collides(tartaglia) || web2.collides(tartaglia) || web3.collides(tartaglia));
    }
